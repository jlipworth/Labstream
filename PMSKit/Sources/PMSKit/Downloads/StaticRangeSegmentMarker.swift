...
/// bytes. Old formats are neither built nor parsed; startup reset owns their cancellation.
public enum StaticRangeSegmentMarker {
    public enum Version: Sendable, Equatable {
        case currentV3
    }

    /// The only supported segment marker.
    ...
    /// Parse the segment's start offset out of a current task description.
    ///
    ...
    public static func parse(_ taskDescription: String?) -> Int? {
        parseTokened(taskDescription)?.offset
    }

    /// Parse a current marker's exact attempt token.
    ...
    private static func parseTokened(
        ...
        guard let taskDescription,
              let r = taskDescription.range(of: prefixV3) else { return nil }
...
public enum DownloadAttemptMarker {
    ...
    public static func attemptIdentity(
        ...
        guard let description,
              let r = description.range(of: prefixV2) else { return nil }
