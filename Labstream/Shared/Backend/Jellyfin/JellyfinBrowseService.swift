...
@MainActor
struct JellyfinBrowseService: MediaBrowserBrowseFacade {
    ...
    func probeSourceBinding(itemId: String) async throws -> Data {
        let context = try context()
        let request = try JellyfinLibrary.itemRequest(server: context.server, token: context.token,
            identity: jellyfinIdentity, userId: context.userID, itemId: itemId)
