...
///
/// `RootView` receives the app-owned services and passes them through the environment.
/// Download ownership is compiled out on tvOS, where Offline is not a product capability.
struct RootView: View {
    ...
    private func handleSystemEntry(_ route: SystemEntryRouter.Route) {
        ...
        systemEntryTask = Task { @MainActor in
            ...
            #if os(macOS)
            // A valid system entry replaces the retained player overlay. Keep this after target
            // (and optional leaf) resolution so a stale/deleted Spotlight route never interrupts
            // active playback, but before navigation/autoplay so the requested detail is visible.
            macPlayerPresenter.dismissForResolvedSystemEntry()
            #endif
