...
struct ContainerBrowserView: View {
    ...
    private var tvBody: some View {
        ...
        #if !os(tvOS)
        .toolbar {
            if ContainerBrowserFeaturePolicy.showsSeasonDownloadAction(
                supportsDownloads: PlatformFeaturePolicy.supportsDownloads,
                childrenAreEpisodes: childrenAreEpisodes,
                hasLoadedChildren: loadState == .loaded && !children.isEmpty
            ) {
...
/// Pure contract for the season browser's only download affordance. The view additionally
/// compiles that affordance and its planner sheet out of tvOS, while this policy gives every
/// target a deterministic assertion over the reachable season-surface conditions.
enum ContainerBrowserFeaturePolicy {
    static func showsSeasonDownloadAction(
        supportsDownloads: Bool,
        childrenAreEpisodes: Bool,
        hasLoadedChildren: Bool
    ) -> Bool {
        supportsDownloads && childrenAreEpisodes && hasLoadedChildren
    }
}
        ...
        HStack(alignment: .top, spacing: compactWidth ? DS.Space.md : DS.Space.lg) {
            ...
            VStack(alignment: .leading, spacing: DS.Space.xs) {
                Text(episodeTitle)
                    ...
                    .accessibilityIdentifier("container.episode.title.\(episode.ratingKey)")
