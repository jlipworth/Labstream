...
final class SystemEntryRouter {
    ...
    #if os(macOS)
    /// Installed by the Mac app delegate. Kept injectable so routing tests can prove that every
    /// system entry activates the unique main window without depending on AppKit window state.
    @ObservationIgnored
    private var mainWindowActivation: (() -> Void)?
    #endif
    ...
    struct Route: Equatable, Identifiable {
        ...
        #else
        init(target: Target, autoPlay: Bool) {
            self.target = target
            self.autoPlay = autoPlay
        }
        #endif
    ...
    #if os(macOS)
    func registerMainWindowActivation(_ activation: @escaping () -> Void) {
        mainWindowActivation = activation
    }

    private func activateMainWindowForSystemEntry() {
    ...
    func open(routeKey: BackendScopedMediaID, autoPlay: Bool) {
        #if os(macOS)
        activateMainWindowForSystemEntry()
        #endif
