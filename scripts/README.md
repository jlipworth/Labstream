...
- `probe-plex-range-drop.sh` — simulator-only Plex download recoverability probe. It launches the DEBUG app in the worktree simulator with the range-drop URLProtocol enabled, using the simulator's signed-in app state and no token env file. Provide `VISIONPLAY_PROBE_QUERY` or `VISIONPLAY_PROBE_RATING_KEY`; logs go under `build/probes/plex-range-drop/`. Add `--keep-app-running` during iterative refactor work when you want the probe to leave the app alive after the observation window.
