#!/usr/bin/env bash
# Simulator-only Jellyfin download probe.
#
# Launches the DEBUG app in this worktree's simulator and uses the simulator's signed-in app state.
# It does not read or print Jellyfin tokens. The app-side probe exercises DownloadManager's real
# Jellyfin path and deletes the probe row after observation unless --keep-record is passed.
set -euo pipefail

usage() {
  cat <<'USAGE'
Usage: scripts/probe-jellyfin-download.sh (--query TEXT | --rating-key ITEM_ID) [options]
...
Options:
  ...
  --start-optimize                  Start the optimize/transcode lane instead.
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --rating-key) [[ $# -ge 2 ]] || { echo "ERROR: --rating-key needs a value" >&2; exit 2; }; rating_key=$2; shift 2 ;;
    ...
    --start-download) start_mode=download; shift ;;
    --start-optimize) start_mode=optimize; shift ;;
...
probe_args=(
  --vp-probe-backend jellyfin
  --vp-probe-jellyfin-download
...
if [[ $start_mode == optimize ]]; then
  probe_args+=(--vp-probe-start-optimize --vp-probe-download-preset "$preset")
else
  probe_args+=(--vp-probe-start-download)
fi
...
probe_status=0
if grep -Eq 'probe\.jellyfin_download\.fail|probe\.fail' "$log_file"; then
  echo "ERROR: probe logged a failure; inspect $log_file" >&2
  probe_status=1
fi
if ! grep -Eq 'probe\.jellyfin_download\.(observe|pass)|downloads\.(enqueue|start|range_start|range_retry|range_checkpoint|range_chunk_appended)' "$log_file"; then
  echo "ERROR: probe did not reach download observation/start; inspect $log_file" >&2
  probe_status=1
fi
...
printf '\n==> Recent probe lines:\n'
grep -E 'probe\.|range-drop|retry|failed|complete|downloads\.(enqueue|start|range_)' "$log_file" | tail -80 || true
exit "$probe_status"
