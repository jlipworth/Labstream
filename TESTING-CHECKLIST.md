...
were collected. Background-session lifecycle events have no `download_id`, so correlate those by
the bounded test interval and require the row-specific events alongside them.
...
| --- | --- | --- | --- |
| Process kill mid-train + background completion delivery | While a static train is active, let the OS terminate/suspend the app (do not swipe-force-quit), leave the device locked/off-head until at least one segment finishes, then relaunch if the OS has not already done so. Pass only if the same row continues from a monotonic durable checkpoint, reaches Complete/Unverified, and no duplicate/gap appears. | For the same hashed ID: `downloads.range_segment_train_adopted` or `downloads.range_dead_finish_adopted`, then `downloads.range_segment_assembled`/`downloads.range_request_rebuild_needed`, and eventually `downloads.complete` plus `downloads.finalize_finished`. OS callback delivery is separately bracketed by `downloads.background_completion_stored`, `downloads.background_session_registered`, `downloads.background_events_finished`, and `downloads.background_completion_fired`. | No diagnostic says why the process died or directly joins a background-session lifecycle event to a download. Capture the termination method and timestamps externally. |
| Resume-blob availability: force-quit versus OS kill | Use two otherwise identical static downloads. In one run swipe-force-quit; in the other induce an OS termination without user force-quit. Relaunch and Resume if offered. Record whether a blob is accepted, rejected as stale, or absent and the durable checkpoint is used. Pass means neither path resets already durable bytes or corrupts the file; do **not** require the two termination modes to preserve equal OS background privileges. | A user Pause first records `downloads.range_pause_resume_data` with `resume_data_present`; actual adoption records `downloads.range_blob_resume`; rejection records `downloads.range_blob_resume_stale`; checkpoint recovery records `downloads.range_auto_resume`, `downloads.range_resume_ready`, or `downloads.range_resume_deferred`. All carry the hashed ID. | There is no event for “resume blob present on launch but not yet consumed,” no termination-mode event, and no explicit “blob absent” event. Blob absence must be inferred from the recovery branch and durable byte count, so this cell is not fully self-proving from diagnostics alone. |
| Wake-time silent retry with an active train | With a train active, lock/remove the device and interrupt reachability long enough for a task to wait or fail; restore reachability while it remains locked/off-head. Pass if retry/continuation advances the same durable checkpoint without user interaction and the train ultimately validates. | `downloads.task_waiting_for_connectivity`, `downloads.range_retry`, `downloads.range_http_retry`, or `downloads.range_blob_resume` identifies the retry path; follow with `downloads.range_segment_assembled`/`downloads.range_progress` and terminal `downloads.finalize_finished`, all on the same hash. | No event records device sleep/wake, app suspension, or that the retry itself occurred while the UI was inactive. Preserve lock/off-head and network-change timestamps externally. |
| Token revoked while asleep | Start a static authenticated download, lock/remove the device, revoke its backend token, and allow the next segment/continuation to run before waking. Pass if stale authorization never produces a Complete row, the durable checkpoint is retained, and the UI ultimately asks for sign-in/retry rather than looping forever. | A rejected Range records `downloads.range_http_rehydrate` with `status_code` 401/403; exhausted handling records `downloads.range_failed` with that status and then `downloads.transfer_auth_dead` for the same hash. If rejection lands in the opaque lane, `downloads.transfer_finished` exposes the HTTP status and the following `downloads.validation_failed` identifies the same row. Emby polling rejection instead uses `downloads.convert_failed` with `status_code` and `phase`. | There is no token-revocation or sleep-state event. Diagnostics prove the HTTP/auth branch and affected row, not when or how the credential was revoked. |
| LAN to WAN switch mid-download | Start on local Wi-Fi, move the physical device onto a genuinely remote path (cellular on iPhone/iPad, or another routed Wi-Fi for Vision Pro), and keep the same train active. Pass if policy permits the new path and bytes remain monotonic, or if policy forbids it the row waits clearly and continues after an allowed path returns. | The hash can show `downloads.task_waiting_for_connectivity`, retry events (`downloads.range_retry`, `downloads.range_http_retry`, or `downloads.range_http_rehydrate`), subsequent `downloads.range_progress`/`downloads.range_segment_assembled`, and finalization. | There is no NWPath/interface, LAN-versus-WAN, expensive/constrained, or cellular-policy decision event. Current diagnostics cannot by themselves prove that a path switch occurred or which interface carried the bytes. |
| Device disk pressure | Fill the device enough to exhaust space during an active static train, without deleting the app/container. Pass if the row fails with the storage-full affordance, keeps only its durable checkpoint, does not finalize a truncated file, and can continue after space is freed and Retry is tapped. | `downloads.range_storage_full` carries the hash and `context` (`task_completion` or an internal start/retry context). A move-stage failure instead records `downloads.move_failed` with a redacted error; final success must still end in `downloads.finalize_finished`. | Free bytes and the exact ENOSPC threshold are not emitted. Record device storage before/after externally; `downloads.move_failed` alone is not storage-specific. |
