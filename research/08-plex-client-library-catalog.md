...
The LukeHagar Speakeasy SDKs (incl. `plexswift`) are **all equally limited** — they expose low-level generated operations (`startUniversalTranscode`, `makeDecision`, `startTranscodeSession`) but **no high-level helper** that assembles the correct bitrate/resolution/profile params for you. Confirmed: the non-Swift ones are the same generator and offer no transcode advantage over plexswift.
...
- All generated from the **same OpenAPI spec via Speakeasy** → identical operation surface per language. Transcoder group: `startTranscodeSession` / `startUniversalTranscode`, `makeDecision`, `transcodeImage`, `transcodeSubtitles`, `triggerFallback`, `getTranscodeSessions`, `stopTranscodeSession`.
