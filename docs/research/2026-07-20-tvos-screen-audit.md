...
| TVUI-024 | Hidden player chrome remote input | After custom chrome auto-hide, directional, Select, and Play/Pause input did not reveal it, so Quality/Subtitles/Audio/Chapters/Speed/Stats could not be reached. The hidden SwiftUI controls leave no reliable focus owner, and `onMoveCommand` does not receive movement outside a focused hierarchy. Keep the shared app-owned `CustomPlayerView`/`CustomPlayerChrome` and custom scrubbing; do not substitute `AVPlayerViewController`. Build a deterministic hidden-chrome fixture, instrument focus/event delivery, and prove the smallest tvOS-only focus/input correction. | Open |
| TVUI-025 | Buffering and player chrome sizing | The buffering dialog and revealed player chrome showed tablet-derived proportions, wrapping, compressed time labels, and weak internal/exterior spacing at 1080p. A buffering-layout adjustment exists, but the full chrome cannot pass visual review until TVUI-024 permits deterministic reveal and submenu traversal. | Fix in progress; full traversal blocked by TVUI-024 |
