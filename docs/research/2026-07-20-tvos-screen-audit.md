...
Status: **active** companion checklist for the parity plan and [issue #246](https://github.com/jlipworth/Labstream/issues/246).
...
| TVUI-004 | Search system keyboard | Selecting a highlighted letter with Select/Enter did not insert it into the query during simulator testing. The failure reproduced with both the original native `.searchable` field and an explicit native SwiftUI `TextField`: the field becomes first responder, but Select clears first responder without inserting a character or updating the query. Live evidence: the field becomes first responder, the keyboard appears, focus reaches the UIKeyboard item; the instant Select lands on a letter, UIKit reloads input views with `responder:(nil)` and `_teardownExistingDelegate:<SwiftUI.TVTextField>` — no character ever reaches the SwiftUI binding. A bare `TextField`, a styled field, a focus-binding replica, and a full generic shell (TabView + NavigationStack + results `ScrollView` + conditional Clear button, the `keyboard-shell` fixture) all pass under fixture UI tests — letters insert fine — which exonerates those layers. Remaining suspects are live-only layers not present in any fixture: `NavigationStack` `.id(activeBrowseSessionKey)`, environment injections, TabView-level `.onExitCommand`, `.task(id:)`, and the `MiniPlayerBar` safeAreaInset. A decisive automated live repro against the real Search tab (`testLiveSearchTabSystemKeyboardInsertsLetter`) exists but has not been run to completion. `.searchable` was tried earlier on tvOS and showed the identical Select-discarded defect — evidence the root bug may be Apple's own (this is a beta OS), not conclusive. Isolate this against a minimal native field and the same beta runtime before assigning the fault to Labstream; do not replace the system keyboard with a custom keyboard. | Open; parked, runtime/app boundary unresolved |
