...
- `Direct Play / Maximum` (`0`) can downshift to `40 Mbps` first, but the automatic up path does not
  re-enter direct play. Direct play is a distinct compatibility/server-load choice, not just another
  bounded rung.
- `Maximum (transcoded)` (`200 Mbps` ceiling) can downshift to `40 Mbps` first and climb back only to
  the highest bounded rung.
