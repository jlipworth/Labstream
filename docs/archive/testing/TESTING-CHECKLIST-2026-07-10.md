# Archived testing checklist — 2026-07-10

> Historical evidence only. This snapshot preserves the former rolling checklist exactly as it
> existed on 2026-07-10; issue-era status, paths, and conclusions below are not current guidance.
> Use the repository-root `TESTING-CHECKLIST.md` for the current manual validation matrix.
...
[`docs/DEVELOPMENT.md`](../../DEVELOPMENT.md) — reminder: a same-bundle-id upgrade install usually preserves the container; deleting the app, erasing the sim, or switching install sources starts fresh and requires sign-in again.

> **Automated coverage map:** this checklist is the manual device/sim pass.
> Current layered validation is summarized in
> [`docs/TESTING-STRATEGY.md`](../../TESTING-STRATEGY.md); live-server probes
...
- [~] **Download integrity + progress** — ⏸️ awaiting final human check. Fresh download progresses,
      ...
      `SIMID=$(scripts/worktree-sim.sh id); xcrun simctl spawn "$SIMID" log show --last 10m --info --debug --predicate 'subsystem == "org.labstream.Labstream"'`
...
- [ ] **Download dual path — OPTIMIZER (offline-download redesign, Phase-0-gated)** — open the sheet
      ...
      `log show --predicate 'subsystem == "org.labstream.Labstream" AND category == "Downloads"'`
...
- [x] **Relaunch mid-flight reconcile (GH #84)** ✅ verified live (2026-06-21) — with a Plex
      ...
      (`xcrun simctl terminate <SIMID> org.labstream.Labstream`) and relaunched: the Plex server-prep
...
- [ ] **Transcode session lifecycle / server-OOM guards (GH #27/#33 reset)** — fix shipped after
      ...
      - [ ] **Stop-before-restart** — quality switch, audio switch, in-player Retry, and a
            ...
            --predicate 'subsystem == "org.labstream.Labstream" AND category == "Playback"'`) and
...
Simulator build/smoke commands live in [`docs/MOBILE-IOS.md`](../../MOBILE-IOS.md). The
