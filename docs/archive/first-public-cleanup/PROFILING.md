...
- subsystem: `org.labstream.Labstream`
...
xcrun simctl spawn "$SIMID" log show --style json --last 15m \
  --predicate 'subsystem == "org.labstream.Labstream" && category == "Performance"' \
  | uv run scripts/perf-log-summary.py --markdown

# If you launched via XcodeBuildMCP, the returned osLogPath is also parseable:
uv run scripts/perf-log-summary.py --markdown < /path/to/org.labstream.Labstream_oslog_*.log
```
