...
| Flow | Unit (mocked/local) | Live probe | Device / manual |
|---|---|---|---|
| Download route decision (original vs. existing version vs. optimizer/convert vs. remux) | `OfflineDownloadDecisionTests`, `PlexDownloadRouterTests`, `JellyfinDownloadRouterTests`, `EmbyDownloadRouterTests`, `DownloadChoicePolicyTests`, `DownloadExistingVersionOptionPolicyTests` | **`LiveDownloadProbe`** (Plex original-vs-optimizer route), **`LiveOptimizeProbe`**, **`LiveEmbyDownloadProbe`** | Checklist B |
| Server-prep handoff (Plex optimize, Emby convert/reuse) | `OptimizePlaylistTests`, `OptimizeTests`, `OptimizedVersionMatchTests`, `EmbyConvertedSourcePolicyTests`, `ServerPrepRefreshPolicyTests`, `ServerPrepAttemptTrackerTests` | `LiveOptimizeProbe`, `LiveSegmentProbe`, `scripts/probe-emby-download.sh --refresh-existing`, `scripts/probe-emby-download.sh --start-optimize` | Checklist B |
| Static byte-range checkpoint/retry/relaunch policy | `RangeTransferHTTPPolicyTests`, `StaticRange*PolicyTests`, `BackgroundRangeCompletionPolicyTests`, `BackgroundDownloadProgressPolicyTests`, `BackgroundDownloadTransientRetryPolicyTests`, `DownloadStaticRetryTargetPolicyTests` | App-driven simulator probes: `scripts/probe-plex-range-drop.sh`, `scripts/probe-jellyfin-download.sh --drop-after-bytes`, `scripts/probe-emby-download.sh --drop-after-bytes` | Checklist B (headset background/off-head still device-only) |
| Download queue, row captions, and progress snapshots | `DownloadProgressDisplayTests`, `DownloadRowDisplayPolicyTests`, `DownloadRowStatusCaptionPolicyTests`, `OfflineLibrarySnapshotBuilderTests`, `DownloadHealthSnapshotPolicyTests`, `OfflineDownloadModelsTests` | **`LiveDownloadStatusProbe`** (read-only queue/progress snapshot) plus app-driven simulator probe observations | Checklist B |
| Emby download path | `EmbyDownloadTests`, `EmbyDownloadRouterTests`, `DownloadBackendRetryIntentPolicyTests`, `EmbyConvertedSourcePolicyTests` | **`LiveEmbyDownloadProbe`**, `scripts/probe-emby-download.sh` | Checklist B (device-only for headset/off-head) |
