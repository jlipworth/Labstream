...
| --- | --- | --- |
| Apple Vision Pro (`Labstream`) | 1.6.1 (build 1) | Neutral-identity preflight archive produced; exact-release-commit archive, validation, upload, and physical acceptance pending. |
| iPhone and iPad (`LabstreamMobile`) | 1.6.1 (build 1) | Neutral-identity preflight archive produced; one final iOS archive covers both device families. Validation, upload, and physical acceptance pending. |
| Apple TV (`LabstreamTV`) | 1.6.1 (build 1) | Unsigned preflight archive produced; Apple Distribution signing, validation, upload, and physical acceptance pending. |
| Mac (`LabstreamMac`) | 1.6.1 (build 1) | Neutral-identity preflight archive produced; exact-release-commit archive, validation, upload, sandbox, and live-host acceptance pending. |
...
4. Upload each archive and wait for App Store Connect processing. Treat warnings or missing
   ...
   the version number.
5. Complete shared app information plus platform-specific metadata, screenshots, build selection,
   privacy/export answers, review notes, and review access.
...
- [ ] Archives are reproducible from the exact public release commit. If that commit changes after
