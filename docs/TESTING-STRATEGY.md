...
Woodpecker provides the repository's public, portable CI surface:
...
There is currently no macOS CI runner for Xcode app builds, app-hosted tests, simulator smoke, or
host-Mac smoke. Those remain local validation gates and must not be inferred from portable CI.
